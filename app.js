...
function initDropdowns() {
  // Curriculum select (student)
  const cs = document.getElementById('curriculumSelect');
  cs.innerHTML = '';
  Object.values(DATA.curricula).forEach(c => cs.add(new Option(c.name, c.id)));

  // Admin curriculum select
...
function autoDetectCurriculum() {
  ...
  if (prefix.length < 2) { hint.textContent = "Curriculum auto-detected from your ID."; return; }

  let matched = null;
  for (const c of Object.values(DATA.curricula)) {
    if (c.idPrefixes && c.idPrefixes.includes(prefix)) { matched = c; break; }
  }
  if (matched) {
    document.getElementById('curriculumSelect').value = matched.id;
    hint.textContent = `Detected: ${matched.shortName} (from ID starting ${prefix})`;
    hint.style.color = 'var(--accent)';
  } else {
    hint.textContent = `ID starts with ${prefix} — please select your curriculum below.`;
...
function loadCourses() {
  ...
  student.name = name;
  student.id = id;
  student.curriculum = document.getElementById('curriculumSelect').value;
